...
main_application.title('Notebook Text Editor')
...
#font family andf font sizw=e functionality
...
file.add_command(label='Save As', image=save_as_icon, compound=LEFT, accelerator='Ctrl+A',command=save_as)
...
def exit_func(event=None):
    ...
    try:
        if text_changed:
            ...
            if mbox is True:
                if url:
                    content = text_changed.get(1.0, END)
                    with open(url, 'w', encoding='utf-8') as fw:
                        fw.write(content)
...
edit.add_command(label='Clear All', image=clear_all_icon, compound=LEFT,accelerator='Ctrl+Alt+X', command=lambda:text_editor.delete(1.0,END))
...
view.add_checkbutton(label='Tool Bar',onvalue=True, offvalue=0, variable = show_toolbar, image=tool_bar_icon, compound= LEFT, command=hide_toolbar)
view.add_checkbutton(label='Status Bar', onvalue=1, offvalue=False, variable = show_statusbar, image=status_bar_icon, compound= LEFT, command=hide_statusbar)

##color button comamnds
def change_theme():
